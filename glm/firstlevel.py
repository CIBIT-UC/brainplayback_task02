...
contrasts.append(
    "Peacefulness + Tenderness + Transcendence + Nostalgia + Power + JoyfulActivation + Tension + Sadness + Wonder - Noise*9"
)
...
contrasts.append(
    "Wonder + Transcendence + Tenderness + Nostalgia + Peacefulness - Noise*5"
)  # sublimity
...
contrasts.append(
    "Wonder*0.2 + Transcendence*0.2 + Tenderness*0.2 + Nostalgia*0.2 + Peacefulness*0.2 - Power*0.5 - JoyfulActivation*0.5"
)  # sublimity vs. vitality
contrasts.append("Power + JoyfulActivation - Tension - Sadness")  # vitality vs. unease
contrasts.append(
    "Tension*0.5 + Sadness*0.5 - Wonder*0.2 - Transcendence*0.2 - Tenderness*0.2 - Nostalgia*0.2 - Peacefulness*0.2"
)  # unease vs. sublimity
...
def glm_function(model, imgs, events, confounds, contrasts, contrasts_renamed, out_dir):
    ...
    for ii in range(len(contrasts)):
        ...
        z_map.to_filename(
            os.path.join(
                out_dir,
                f"{subject}_task-{task_label}_stat-z_con-{contrasts_renamed[ii]}.nii.gz",
            )
        )
        ...
        beta_map.to_filename(
            os.path.join(
                out_dir,
                f"{subject}_task-{task_label}_stat-beta_con-{contrasts_renamed[ii]}.nii.gz",
            )
        )
        ...
        plt.savefig(
            os.path.join(
                out_dir,
                f"{subject}_task-{task_label}_plot-z_con-{contrasts_renamed[ii]}_c-fdr-0.05_clusterk-10.png",
            )
        )

        # cluster table
        table = get_clusters_table(z_map, threshold, 10)
        ...
        table.to_csv(
            os.path.join(
                out_dir,
                f"{subject}_task-{task_label}_table-clusters_con-{contrasts_renamed[ii]}_c-fdr-0.05_clusterk-10.tsv",
            ),
            sep="\t",
        )
