...
def estimate_stability_perfold(feat_dir, output_stab_dir, subject):
    """
    ...
    # check number of runs aka folds
    n_folds = len(stab_feat_files)
    if n_folds != 4:
        raise ValueError(f"Expected 4 folds, found {n_folds}.")
    
    # Each fold needs to have 3 runs
    # Let's iterate over the folds
    for ff in range(n_folds):

        # Excluding the run with index ff
        stab_feat_files_fold = stab_feat_files.copy()
        del stab_feat_files_fold[ff]

        # check number of runs per fold
        if len(stab_feat_files_fold) != 3:
            raise ValueError(f"Expected 3 runs per fold, found {len(stab_feat_files_fold)}.")
        ...
        if stab_feat.shape[-1] != 3*2*2:
            print(f"WARNING: Expected 12 trials per condition, found {stab_feat.shape[-1]}.")
        ...
        # Save STAB
        np.save(os.path.join(output_stab_dir, f'sub-{subject}_fold-{ff}_STAB.npy'), STAB)

        print(f"Done estimating stability for subject {subject} fold {ff}.")
...
def convert_samples_to_features_perfold(samples, output_func_dir, events_split, subject, run, fold):

    print(f'Converting samples to features for subject {subject}, run {run}, fold {fold}...')
    ...
    # save features
    np.save(os.path.join(output_func_dir, f'sub-{subject}_ses-01_task-02a_run-{run}_fold-{fold}_features.npy'), features)

    # save the labels ('trial_type' from events_split)
    labels = events_split['trial_type'].values
    np.save(os.path.join(output_func_dir, f'sub-{subject}_ses-01_task-02a_run-{run}_fold-{fold}_labels.npy'), labels)

    print(f'Features extracted and labels saved for subject {subject}, run {run}, fold {fold}.')
    #return features
