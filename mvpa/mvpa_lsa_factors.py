...
dataset_files = [os.path.join(dataset_dir, f) for f in os.listdir(dataset_dir) if f.endswith('_factors_dataset.nii.gz')]
dataset_files.sort()

# find all *_trial_types.txt files in dataset_dir
label_files = [os.path.join(dataset_dir, f) for f in os.listdir(dataset_dir) if f.endswith('_factors_trial_types.txt')]
label_files.sort()
...
print('Accuracy per fold:', scores['test_score'], sep='\n')

# %% export results scores
import pandas as pd
results = pd.DataFrame(scores)
results.to_csv(os.path.join(dataset_dir, 'mvpa-lsa-factors-results.csv'), index=False)
print('saved accuracy results to mvpa-lsa-factors-results.csv')
# import pandas as pd
