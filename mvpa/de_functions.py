...
# %%
def ls_a_factors(root_dir, subj, task, run):
...
def ls_a_full_confounds(root_dir, subj, task, run):
    ...
    # Add counter to each trial_type, except for Noise, in the format '01'
    # start a dict with a counter for each trial_type
    ...
    for contrast in trialwise_conditions:
        z_map = lsa_glm.compute_contrast(f'{contrast} - Noise', output_type='z_score')
    ...
    # check if image exists - if so delete it
    newimage = os.path.join(output_dir, subj + '_ses-01_task-' + task + '_run-' + run + '_full_confounds_dataset.nii.gz')
    ...
    # check if trial_types file exists - if so delete it
    newtrialtypes = os.path.join(output_dir, subj + '_ses-01_task-' + task + '_run-' + run + '_full_confounds_trial_types.txt')
