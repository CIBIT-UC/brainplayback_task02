...
dataset_files = [os.path.join(dataset_dir, f) for f in os.listdir(dataset_dir) if f.endswith('_full_confounds_dataset.nii.gz')]
dataset_files.sort()

# find all *_trial_types.txt files in dataset_dir
label_files = [os.path.join(dataset_dir, f) for f in os.listdir(dataset_dir) if f.endswith('_full_confounds_trial_types.txt')]
label_files.sort()
...
results1.to_csv(os.path.join(dataset_dir, 'mvpa-lsa-full-gm-results-1.csv'), index=False)
print('saved accuracy results to mvpa-lsa-full-gm-results-1.csv')
